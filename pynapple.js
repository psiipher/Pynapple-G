...
function displayLines() {
	fetch('http://localhost:8000/data/ddcel/CaliforniaRoads.geojson')
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
		}
...
function displayDDCEL() {
	fetch('http://localhost:8000/data/ddcel/DDCEL.geojson')
    .then(response => {
        if (!response.ok) {
            throw new Error('Network response was not ok');
		}
